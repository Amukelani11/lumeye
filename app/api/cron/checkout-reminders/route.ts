...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const abandonment of firstReminders || []) {
      try {
        if (abandonment.email) {
          ...
          emailsSent++
          console.log(`Sent first reminder to ${abandonment.email}`)
        }
      } catch (error) {
    ...
    for (const abandonment of secondReminders || []) {
      try {
        if (abandonment.email) {
          ...
          emailsSent++
          console.log(`Sent second reminder to ${abandonment.email}`)
        }
      } catch (error) {
    ...
    console.error('Checkout reminders cron Error:', error)
    return NextResponse.json(
